...
public class Info {
    ...
    Info merge (Info inputList) {
        ...
        while (thisCurrentNode != null || inputCurrentNode != null) {
        	if (inputCurrentNode == null || (thisCurrentNode != null 
        	&& thisCurrentNode.data.compareTo(inputCurrentNode.data) < 1 )) {
            	...
            	//If the two nodes are equal, update currentNodeB as well so node is not 
            	//reinserted 
    ...
    /**
     * Return an Info object that is a copy of the contents from the first parameter 
     * position to the last parameter position.  If the bounds given are out of bounds,
     * return null.
    ...
    /**
     * Remove a list of items from this info list.  
     * The result is that items common to this Info object and the input Info object are 
     * removed from this Info object.
     * @param itemsToDelete Info object to containing items to remove from this Info object
     */
    public void delete (Info itemsToDelete) {
    	...
    	while (currentNode != null && deleteNode != null) {
    		if (previousNode == null && currentNode.data.compareTo(deleteNode.data) == 0) {
    		...
    		//Else no match, so select smaller next node, and set appropriate reference
    		else if (currentNode.data.compareTo(deleteNode.data) < 0) {
    			previousNode = currentNode;
    			currentNode = currentNode.next;
    		}
