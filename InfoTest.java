...
public class InfoTest {
	...
	public void testInsert() {
		...
		assertEquals("incorrect number of items in array", 4,
                             info.count());
		//test ascending for order
		assertEquals("array out of order", "A" +
                             DELIMITER + "1", info.get(0).toString());
		assertEquals("array out of order", "B" +
                             DELIMITER + "2", info.get(1).toString());
		assertEquals("array out of order", "C" +
                             DELIMITER + "3", info.get(2).toString());
		assertEquals("array out of order", "D" +
                             DELIMITER + "4", info.get(3).toString());
		//insert an item already in the array (should fail)
		info.insert(i1);
		assertEquals("invalid item added to start of array", 4,
                             info.count());
		info.insert(i3);
		assertEquals("invalid item added to middle of array", 4,
                             info.count());
		info.insert(i4);
		assertEquals("invalid item added to end of array", 4,
                             info.count());
	}
	...
	public void testCopy() {
		...
		assertEquals("failed to copy first item", "B" +
                             DELIMITER + 2, i.get(0).toString());
		//copy middle two items
		i = info.copy(1, 2);
		assertEquals("failed to copy middle items", 2, i.count());
		assertEquals("failed to copy middle items", "B" +
                             DELIMITER + "2", i.get(0).toString());
		assertEquals("failed to copy middle items", "C" +
                             DELIMITER + "3", i.get(1).toString());
		//copy last item
		i = info.copy(3, 3);
		assertEquals("failed to copy last item", 1, i.count());
		assertEquals("failed to copy last item", "D" +
                             DELIMITER + "4", i.get(0).toString());
		//copy entire array
		i = info.copy(0, 3);
		assertEquals("failed to copy entire array", 4, i.count());
		assertEquals("failed to copy entire array", "A" +
                             DELIMITER + "1", i.get(0).toString());
		assertEquals("failed to copy entire array", "B" +
                             DELIMITER + "2", i.get(1).toString());
		assertEquals("failed to copy entire array", "C" +
                             DELIMITER + "3", i.get(2).toString());
		assertEquals("failed to copy entire array", "D" +
                             DELIMITER + "4", i.get(3).toString());
		//test out of bounds
		//x < 0
		i = info.copy(-1, 0);
	...
	public void testMerge() {
		...
		assertEquals("failed to correctly sort merged list", "A" +
                             DELIMITER + "1", mergedList.get(0).toString());
		assertEquals("failed to correctly sort merged list", "B" +
                             DELIMITER + "2", mergedList.get(1).toString());
		assertEquals("failed to correctly sort merged list", "C" +
                             DELIMITER + "3", mergedList.get(2).toString());		
	}
	...
	public void testDeleteInfoList() {
		...
		assertEquals("failed to correctly delete info list", 3,
                             info.count());
		assertNotEquals("failed to correctly delete info list", "A" +
                                DELIMITER + "0", info.get(0).toString());
		...
		assertEquals("failed to correctly delete info list", "E" +
                             DELIMITER + "4", info.get(2).toString());
		...
		info.delete(infoB);
		//verify delete functionality
		assertEquals("failed to correctly delete info list", 0,
                             info.count());
